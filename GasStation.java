...
/*
iven two integer arrays A and B of size N. There are N gas stations along a circular route, where the amount of gas at station i is A[i].
...
public class GasStation {
    ...
    public static int canCompleteCircuit(final int[] A, final int[] B) {
        ...
        while (count < A.length) {
            ...
            gasInCar = gasInCar + A[i];
            if (gasInCar - B[i] < 0) {
                i++;failCount++;count=0;
                if(failCount==A.length){
                    start=-1; break;}
                gasInCar = 0;
            } else {
                if (count == 0) {
...
//                System.out.println("Gas = "+gasInCar+"  Required= "+B[i]);
                gasInCar = gasInCar - B[i];
                i++;
                count++;
                if (i == A.length) {
                    i = 0;
